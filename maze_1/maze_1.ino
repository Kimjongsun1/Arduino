...
{
if((right_sonar >= 0)&&(right_sonar <= 200)&&(front_sonar >= 0)&&(front_sonar <= 200)&&(left_sonar >= 0)&&(left_sonar <= 200))
{
  maze_status = 4;
...
/*
  ...
  if (front_sonar <= 350.0)
  {
    //정지
    Serial.println("Rotate stop");
    motor_B_control(HIGH, 0); //오른쪽 정지
    motor_A_control(LOW, 0); //왼쪽은 정지
    delay(500);                //일정한 시간 동안 회전

    //180도 반회전 시계 방향으로 회전
    Serial.println("Rotate CCW");
    motor_B_control(HIGH, 100); //오른쪽 전진
    motor_A_control(LOW, 100); //왼쪽은 후진
    delay(705);                 //일정한 시간 동안 회전
