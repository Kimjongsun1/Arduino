...
void Excersize_2_3(int solution, int answer) // 한라대학교 미래모빌리티공학과 김종선
{
	...
	if (solution == SHOW)
	{
		...
		printf("따라서 평균 속력의 공식은 : vavg = (Δx / Δt)입니다 \n\n");
		printf("vavg : 평균속력, Δx  : 변위, Δt : 시간변화량\n");
		printf("Δx : (xf - xi), Δt : (tf - ti)");
		printf("나중거리 xf는 %6.2lfkm이고, 처음거리 xi는 %6.2lfkm입니다. \n", distance, distance_0);
		printf("나중시간 tf는 %6.2lfh %6.2lfm이고, 처음시간 ti는 %6.2lfh입니다.\n", time_h, time_m, time_0);
		printf("평균속력은 %6.2lfkm - %6.2lfkm / %6.2lfh %6.2lfm - %6.2lfh이며\n", distance, distance_0, time_h, time_m, time_0);
		printf("km/h 를 m/s로 변환해보면\n");
		printf("km는 m에 1000을 곱하면 되고,\n'h'에 3600을 곱하여 's'를 구하고 'm'에 60을 곱하여 's'를 구힙니다\n");
		printf("따라서 %6.2lfkm = %6.2lfkm * 1000 = 408000m이고\n2h 37m = 2h * 3600 + 37m * 60 = %6.2lfs입니다\n", distance, distance, time_f);
		printf("km/h에서 m/s로 변환한 값을 거리를 시간으로 나누면 됩니다\n");
		printf("따라서 (%6.2lf x 1000)m / (%6.2lf)s = %6.2lfm/s이므로\n", distance, time_f, velocity);
