...
void imu_rotation(void)
{
 ...
 if(heading_angle_error > THERSHOLD_ANGLE) //반시계 방향으로 회전
  {
    motor_R_control(70);
    motor_L_control(-70);
  }

  else if((heading_angle_error >= THERSHOLD_ANGLE) && ( heading_angle_error <= THERSHOLD_ANGLE)) // 정지
  {
    motor_R_control(0);
    motor_L_control(0);
  }
